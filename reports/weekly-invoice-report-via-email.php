...
if ( ! defined( 'POP_SEND_XML_INVOICE_DAY_OF_WEEK' ) ) {
    define( 'POP_SEND_XML_INVOICE_DAY_OF_WEEK', 1 ); // Lunedì
}
if ( ! defined( 'POP_SEND_XML_INVOICE_TIME_OF_DAY' ) ) {
    define( 'POP_SEND_XML_INVOICE_TIME_OF_DAY', "8:00:00" ); // Ore 8:00:00
}
if ( ! defined( 'POP_SEND_XML_INVOICE_EMAIL_FROM_NAME' ) ) {
    define( 'POP_SEND_XML_INVOICE_EMAIL_FROM_NAME', get_bloginfo( 'name' ) );
}
if ( ! defined( 'POP_SEND_XML_INVOICE_EMAIL_FROM_EMAIL' ) ) {
    define( 'POP_SEND_XML_INVOICE_EMAIL_FROM_EMAIL', 'noreply@eventibelli.it' );
}
if ( ! defined( 'POP_SEND_XML_INVOICE_EMAIL_TO' ) ) {
    define( 'POP_SEND_XML_INVOICE_EMAIL_TO', 'email@commercialista.it' );
}
...
function send_email_with_invoices() {
    ...
    if ( ! empty( $attachments ) ) {
        ...
        if ( ! empty( $checkSentInvoice ) ) {
            ...
            error_log( 'ATTENZIONE: Email cron non inviata, allegati non presenti' );
        }
    }

    /* DEBUG */
    if ( empty( $attachments ) ) {
        $msg = "Nessun allegato per gli ordini: " . print_r( $orders, true );
    } else {
        $msg = "Allegati per gli ordini: " . print_r( $attachments, true );
    }
}
